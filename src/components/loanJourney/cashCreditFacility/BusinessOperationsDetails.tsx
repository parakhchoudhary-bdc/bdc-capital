...
// Define validation rules for business operations & financials
const validationRules: Record<string, z.ZodTypeAny> = {
  ...
  primaryOperatingCycle: z
    .string()
    .min(1, "Please select primary operating cycle"),
  averageOperatingCycleDuration: z
    .string()
    .min(1, "Please select average operating cycle duration"),
  gstNumber: z
    .string()
    .optional()
    .or(z.literal(""))
...
interface BusinessOperationsDetailsProps {
  formConfig?: InputField[];
  onBack?: () => void;
  onNext?: () => void;
}

const BusinessOperationsDetails: React.FC<BusinessOperationsDetailsProps> = ({
  formConfig = [],
  onBack,
  onNext,
}) => {
...
export default BusinessOperationsDetails;
